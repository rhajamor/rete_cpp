...
struct vect3
{
    ...
    typename = typename std::enable_if<std::is_floating_point< scalar_type>::value>::type
    >
    vect3<fp_type> operator*(const scalar_type scalar) const
    {
        vect3<fp_type> ret;
        ret._array = {
            _array[0] * scalar,
    ...
    fp_type  operator*(const vect3<fp_type> &vect1) const
    {
        vect3<fp_type> ret;
    ...
    vect3<fp_type> operator-(const vect3<fp_type> &vect1) const
    {
        vect3<fp_type> ret;
    ...
    fp_type operator[](const int index) const
    {
        return _array[index];
    }
};
...
struct Quaternion
{
    ...
    Quaternion<fp_type> operator*(const Quaternion<fp_type> &q) const
    {
...
int main()
{
    vect3<std::complex<double>> v;
    v._array = {1.75, -4., -5.};
    Quaternion<double> q1(3, v);
    auto prod = (q1 * q1);
    std::cout << "prod.w = " << prod.w << " prod.x = " << prod.v[0] << " prod.y = " << prod.v[1] << " prod.z = " << prod.v[2] << " !\n";
